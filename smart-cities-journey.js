// smart-cities-journey.js — wires "city-card" cards into the shared My Journey bookmark + search system
    ...
    bookmarkButtons.forEach((btn) => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        window.Journey.toggle({
          id, explorerPage: 'smart-cities.html', title, thumbnail, category
        });
        setPressed();
      });
    });

    window.Journey.registerSearchItems('smart-cities.html', cards.map((card) => ({
      id: card.dataset.id,
      title: card.querySelector('h2, h3, h4')?.textContent.trim() || 'Item',
      description: card.querySelector('p')?.textContent.trim() || '',
      link: 'smart-cities.html'
    })));
  }
